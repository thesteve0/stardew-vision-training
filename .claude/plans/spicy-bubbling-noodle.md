...
**CPU time estimate**: ~4-8 hours for 10K images × 50 epochs. Can reduce to 20 epochs if val accuracy plateaus early.
...
| Synthetic↔real domain gap | Medium | Augmentations (JPEG, noise, blur) target screenshot artifacts; can add a few real text crops to training if needed |
